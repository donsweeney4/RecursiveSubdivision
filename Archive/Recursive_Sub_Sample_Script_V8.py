...
def create_folium_map_with_layers(
    ...
    # Contour + White Mask overlays
    if add_centroid_contour:
        try:
            bounds, mask_path, contour_path = save_contour_image(
                subregion_gdf,
                image_filename=centroid_contour_png,
                output_dir="."
            )

            # Make paths relative to the HTML so the browser can find them
            html_dir = os.path.dirname(os.path.abspath(output_html)) or "."
            contour_rel = os.path.relpath(os.path.abspath(contour_path), start=html_dir)
            mask_rel    = os.path.relpath(os.path.abspath(mask_path),   start=html_dir)

            ImageOverlay(
                name="Centroid Contours (linear)",
                image=contour_rel,
                bounds=bounds,
            ...
            ImageOverlay(
                name="Void Regions (White Mask)",
                image=mask_rel,
                bounds=bounds,
                opacity=1.0,          # fully opaque white to cover contour below
                interactive=False,
                zindex=3
            ).add_to(m)
...
def save_contour_image(
    ...
    """
    Save a transparent contour PNG clipped to kept subregions AND
    a separate white mask PNG for dropped/void regions.

    Returns:
      bounds (for Folium ImageOverlay), mask_image_path
    """
    from scipy.interpolate import griddata
    ...
    mask_path = os.path.join(
        output_dir, os.path.splitext(image_filename)[0] + "_mask.png"
    )
    ...
    x_min, y_min, x_max, y_max = subregion_gdf.total_bounds
    full_bbox = Polygon([(x_min, y_min), (x_max, y_min), (x_max, y_max), (x_min, y_max), (x_min, y_min)])
    void_union = full_bbox.difference(keep_union) if keep_union is not None else full_bbox

    # Transparent out void pixels in the contour
    try:
        if not void_union.is_empty:
            void_prep = prep(void_union)
            pts = (Point(x, y) for x, y in zip(X.ravel(), Y.ravel()))
            void_mask = np.fromiter((void_prep.covers(p) for p in pts), dtype=bool, count=X.size)
            Z[void_mask.reshape(X.shape)] = np.nan
            print("✅ Applied transparent mask to void regions.")
    except Exception as e:
        print(f"⚠️ Masking step failed: {e}")

    # Save contour PNG (transparent background)
    fig, ax = plt.subplots(figsize=(8, 8), dpi=150)
    ax.set_axis_off()
    ax.set_xlim(xi.min(), xi.max())
    ax.set_ylim(yi.min(), yi.max())
    ax.set_position([0, 0, 1, 1])
    ax.contourf(X, Y, Z, levels=20, cmap=HIGH_CONTRAST_CMAP)
    plt.savefig(contour_path, bbox_inches='tight', pad_inches=0, transparent=True)
    plt.close(fig)

    # Save separate white mask PNG (opaque white)
    if void_union is not None and not void_union.is_empty:
        fig2, ax2 = plt.subplots(figsize=(8, 8), dpi=150)
        ax2.set_axis_off()
        ax2.set_xlim(xi.min(), xi.max())
        ax2.set_ylim(yi.min(), yi.max())
        ax2.set_position([0, 0, 1, 1])
        geoms = list(void_union.geoms) if hasattr(void_union, "geoms") else [void_union]
        for g in geoms:
            if g.is_empty:
                continue
            x, y = g.exterior.xy
            ax2.fill(x, y, color="white", alpha=1.0, zorder=1)
        plt.savefig(mask_path, bbox_inches='tight', pad_inches=0, transparent=True)
        plt.close(fig2)
        print(f"✅ Created separate white mask overlay: {mask_path}")

    bounds = [[float(yi.min()), float(xi.min())], [float(yi.max()), float(xi.max())]]
    print(f"✅ Contour image from {src}: {contour_path}  bounds={bounds}")
    return bounds, mask_path, contour_path
...
def build_pipeline(
    ...
    # Filter out rectangles with < 4 samples
