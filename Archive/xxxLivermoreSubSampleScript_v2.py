...
def load_points_csv(path: str) -> gpd.GeoDataFrame:
    """
    Load a CSV with at least: longitude, latitude, temperature.
...
def compute_average_temperature(
    ...
    """
    Compute avg temperature and sample count for each subregion polygon.
...
def create_folium_map_with_layers(
    ...
    tooltip = folium.GeoJsonTooltip(
        fields=["avg_temperature", "sample_count"],
        aliases=["Avg Temp (°F):", "Samples:"],
        localize=True,
        sticky=True
    )
...
def build_pipeline(
    ...
    create_folium_map_with_layers(subregions, output_html=folium_html)
